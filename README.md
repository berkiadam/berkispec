...
- [Berki-spec](#berki-spec)
  ...
  - [5. Teljes berki spec flow (00–09)](#5-teljes-berki-spec-flow-0009)
    ...
    - [5.5 Az 07-validate önjavító hurok (részletes) — tesztek + kódreview](#55-az-07-validate-önjavító-hurok-részletes--tesztek--kódreview)
    - [5.6 Önjavító hurkok (analyze + validate) — közös konvenciók](#56-önjavító-hurkok-analyze--validate--közös-konvenciók)
    - [5.7 Példa prompt-folyam (egy ciklus végigvezetése)](#57-példa-prompt-folyam-egy-ciklus-végigvezetése)
  ...
  - [12. docs-generated/ — élő dokumentáció (a 08-doc-sync gazdája)](#12-docs-generated--élő-dokumentáció-a-08-doc-sync-gazdája)
    - [12.1 specs/test-conventions.md — visszatérő teszt-elvárások és receptek (TC1–TC11)](#121-specstest-conventionsmd--visszatérő-teszt-elvárások-és-receptek-tc1tc11)
  ...
  - [17. Validációs riport (validation-report.md)](#17-validációs-riport-validation-reportmd)
...
└── prompts/
    ...
    ├── shared/                       # Skillek ÉS agent-promptok közötti megosztott szövegblokkok (build-time inline)
    ...
    │   ├── phase-commit.md           # közös fázis-záró commit eljárás (PC1: user-jóváhagyás → státuszírás → kötelező commit + ellenőrzés); a 02/03/04/05/07 hivatkozza
    ...
    │   ├── analyze-gate-check.py     # a 05-analyze mechanikus kapuja: plan-`[P-…]` ↔ task-hivatkozás, marker, `⟂`, `DoD-NN`, kötelező táblák, futtatott artefaktumok (A1), plan-horgonyok (A2), artefaktum-hang (A3), a DoD→plan→task lefedettségi lánc (C1–C3), KF1 üres cellák (C4), task-határon átnyúló shell-változó (C5) + a KÉT GENERÁLT RIPORT-TÁBLA és a `## Leltár` az analyzereknek (AG3/AG4)
    ...
    │   ├── round-log.py              # 07: a validation-report.md `## Kör N` blokkjainak nyitása/zárása + a round-NN mappa (VD9, TR5)
    │   ├── validate-gate-check.py    # 07: gyűjtőkapu — státuszok, nyitott taskok/DoD-pipák, IP1, nyitott Must Fix, kör-blokk ↔ mappa egyezés
    │   ├── dod-check.py              # 07: DoD ↔ bizonyíték join a kör JUnit-eredményeivel (DI1/DI2)
    │   ├── contract-guard.py         # 07: VD3a szerződés-integritás kapu — védett útvonalak + csalás-minták a diffben
    │   ├── sonar-gate.py             # 07: Sonar Quality Gate az API-ból (QG státusz, bukott feltételek, BLOCKER/CRITICAL/MAJOR) — QG1 külön kilépő kóddal
...
- **Forrás:** [`prompts/models.json`](prompts/models.json) — platformonként (`claude` / `antigravity` / `copilot` / `cursor` / `codex`) a 3 tier `{model, effort}` objektumként, plusz a defaulttól eltérő agentek **saját nevű sorként** (csak az `effort` mezővel; a modelljük a `default` tierből jön). Az `install-helper.py` `AGENT_MODEL_KEYS` szótára rendeli az `analyzer`/`researcher`/`10-cycle-status` stemeket a tierekhez (az `analyzer-exec` szándékosan nincs benne: a kapu leltára készen adja neki a jelölteket, tehát `default` tieren fut); ami nincs sem itt, sem saját sorként a `models.json`-ban, `default` modellt és `default` (=`high`) effortot kap.
...
Ez az ábra **kizárólag az 05-analyze lépést** mutatja be, a subagentek és a kérdés-folyam feltüntetésével. Az orchestrátor (05-analyze) read-only: a **gépies réteget** a `analyze-gate-check.py` kapu, a **szemantikai diagnózist** az `analyzer` (ez az egyetlen pont a teljes rendszerben, ami a legdrágább, `deep_reasoning_agent` tier-en fut — lásd 5.3), a **végrehajthatósági diagnózist** az `analyzer-exec` (`default` tier, vele **párhuzamosan**), a **javítást** a fixer-subagentek (02/03/04 fix-mód, `default` tier) végzik; a felhasználót mindig az **orchestrátor** (szintén `default` tier) kérdezi, fázis-jelzéssel.

Két rövidítő ág van benne, mert ezek adják a fázis megtakarításának nagy részét: a fixer után **először a kapu fut** (G) — ha csak mechanikus hibát talált, az visszamegy ugyanahhoz a fixerhez, analyzer-kör és iteráció-fogyasztás nélkül —, és ha a fixer **semmit nem változtatott** (N), a hurok nem indít analyzert, hanem megáll és kérdez.
...
flowchart TD
    ...
    AZ["<b>analyzer</b> ‖ <b>analyzer-exec</b><br/>PÁRHUZAMOS subagentek<br/>(read-only diagnózis:<br/>1–5. és 6. kategória)"]:::agent
    ...
    MechOnly{"4.b Csak mechanikus<br/>hiba? (G)"}:::decision
    Down{"5. downstream-hatás:<br/>van? (D11)"}:::decision

    O -- "⓿ minden kör előtt" --> GATE
    GATE -- "① Leltár → analyzer-exec<br/>generált mátrix → analyzer" --> AZ
    AZ -- "② két megállapítás-lista" --> O
...
### 5.5 Az 07-validate önjavító hurok (részletes) — tesztek + kódreview
...
### 5.6 Önjavító hurkok (analyze + validate) — közös konvenciók
...
### 5.7 Példa prompt-folyam (egy ciklus végigvezetése)
...
| `/bs-analyze` | Analyze | ciklus mappa | `analyze-report.md` (PASS/FAIL) — mechanikus kapu + **két párhuzamos diagnoszta** (`analyzer` 1–5. kategória, `analyzer-exec` 6. kategória); a két lefedettségi táblát a kapu **generálja**. FAIL esetén orchestrált önjavító hurok (fixer-subagentek, `max X=3`, iterációnként **egy** analyzer-kör) |
...
### 12.1 specs/test-conventions.md — visszatérő teszt-elvárások és receptek (TC1–TC11)
...
## 17. Validációs riport (validation-report.md)
...
### 19.0 Platform-korlát: parancs-futtatás a subagentekben (EX1)
