...
2. **`analyzer`** (read-only) — a szemantikai kategóriák a `spec.md` ↔ `plan.md` ↔ `tasks.md` ↔ `conventions.md` négyesen: duplikáció, ambiguitás, alulspecifikáció, konvenció-ütközés, a lefedettség **tartalmi** ítélete a generált mátrixon;
3. **`analyzer-exec`** (read-only, az előzővel **párhuzamosan**) — végrehajthatóság és artefaktum-tulajdon a `plan.md` + `tasks.md` + leltár hármasból.

Iterációnként **egy** teljes analyzer-kör fut (a `PASS` csak teljes körből adható), és két rövidítő ág védi az iteráció-számot: a fixer után **először a kapu fut** — mechanikus regresszió esetén az visszamegy ugyanahhoz a fixerhez, analyzer-kör nélkül (G) —, és ha a fixer semmit nem változtatott, a hurok megáll és kérdez (N). Ugyanez a kapu a `03` és `04` fázis **lezárása előtt** is lefut (M), hogy a gépies hibák a keletkezésük helyén derüljenek ki. FAIL esetén az 05 **orchestrátorként önjavító hurkot vezényel**: a legkorábbi érintett célfázishoz indít egy fixer-subagentet (a 02/03/04 fix-módja), downstream re-deriválás (`02→03→04`, célzott reconciliation) után újra-analyze fut — amíg PASS, vagy `max X=3` iterációig. A döntést igénylő kérdéseket a fixerek a `*-questions.md`-be gyűjtik (nem kérdeznek közvetlenül), és az orchestrátor teszi fel a felhasználónak (`FÁZIS/Knn` fejléccel); nyitott kérdésnél a hurok megáll, válasz után folytatódik. A hurok alatt a dokumentumok `[analyze-loop]` státusz-markert viselnek (auto-státusz + megszakítás-biztos folytatás), és csak a hurok végén készül egyetlen commit.
