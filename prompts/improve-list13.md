...
|---|---|---|
| `Test manager` | `none` (alap) · `testdino` · `reportportal` *(under development)* · `qase` *(under development)* | melyik adapter fut; `none` esetén a többi mező elhagyható. **A végrehajtás állapota (2026-09-22):** a `testdino` éles fiókkal mérve, a `reportportal`/`qase` **under development** — a kód tartja a `TM6` szerződést, de valódi szolgáltató ellen még nem futott |
...
| 2026-09-22 | **Utó-audit: három agent-kontraktus hézag javítva.** A 9.c minden tétele kész volt, de a `7/o` kör tanulsága szerint (*„a `reviewer` fájlt ír egy nem létező mappába; a `researcher` `called_by` listája nem is tartalmazta a quick-flow-t"*) külön ellenőriztem az **ágens-szerződéseket**, és ugyanaz a hibaosztály bukkant fel: (a) a `reviewer` `called_by` listája nem tudott a `09b-review`-ról, és a törzse egyetlen hívót (a `07`-et) és egyetlen kimenetet (`code-review.md`) feltételezett — mostantól kimondja a **kontraktus-helyettesítés** hármas esetét, és hogy **a hívóé az elsőbbség**; (b) a `shared/fix-mode-implement.md` csak két javító-szekciót ismert — bekerült a `## Post-merge javítások` ág a prerequisite-jével; (c) az `implement-fixer` bemenete bedrótozva a `## Validációs javítások` volt — mostantól „az **aktív** javító-szekció, amit a hívó mond meg", mert az `auto-fix-loop` ágon a ciklusvég merge-ága indítja. A gemini `agent.json` tükrök újragenerálva. **Ugyanitt:** az `E9` „kipróbálatlan adaptert jelölni **a dokumentációban**" követelménye eddig csak a scriptben teljesült — bekerült a `00` sablon-prózájába, a README-kbe és a `berki-spec-directory-structure.md`-be, hogy a `reportportal`/`qase` ág kipróbálatlan. |
| 2026-09-22 | **A `reportportal` és a `qase` adapter `under development` jelölést kapott** (a korábbi „kipróbálatlan" helyett). A különbség nem kozmetikai: a jelölés azt mondja ki, hogy **a kód megvan és a `TM6` szerződést tartja, de valódi szolgáltató ellen soha nem futott** — használható, de a feltöltő hívást a szolgáltató *aktuális* CLI-jével validálni kell, és addig **ne tegyék kapuvá** (`Test manager kötelező: igen`). A `test-manager.py` `ADAPTER_STATUS` táblát kapott (`ready` / `under-development`), a fejlécében `[UNDER-DEVELOPMENT]` jelöléssel, és **minden módban** (nem csak a preflightban) hangos figyelmeztetéssel. Átvezetve: a `00` érték-listája és interjú-kérdése mindkét nyelven, a README-k, a `berki-spec-directory-structure.md`, a `meta-improve-prompts.md` script-táblája és az 5.b mezőtábla. |
| 2026-09-22 | **A `meta-improve-prompts.md` utó-auditja.** Az `F10` task első körben a fázis-leírást, a fájl-táblát, a script-táblát és a „még nincs végrehajtva” figyelmeztetést vezette át — az audit viszont négy továbbit talált: (a) a nyitó mondat még **11 skillt** mondott (most 15, a ciklusvég családjával); (b) a `7/i` elv a `PH1`-et a **régi** alakjában írta le (`mindkettő`, „üres cella = mindkettő”) — ez egy új sessionnek **hamis szabályt tanított volna**, ezért a bekezdés most kimondja, hogy ez az alak az `L13-D3` óta megszűnt, és hogy miért (a `post-merge` kör konténerizálás-követelménye); (c) a shared-tábla `review-checklist` sora és a `run-tests.py` / `report-gate-check.py` / `cycle-status.py` script-sorai nem ismerték az új hívókat (a ciklusvég merge-ága, a `bs-dev-test`, a fázis-záró commit); (d) a `7/e` riport-fázis bekezdése nem tudott a `post-merge`/`dev-test` értékről, a `test-report/` zárt listája pedig a két új mappáról. **Új, HATODIK kötelező ellenőrzés került a záró listába:** ha ágenst hívó szabályt írsz, naprakész-e az agent **szerződése** (`called_by` + kontraktus-helyettesítés, QF18) — pontosan az a hibaosztály (`7/o`), ami ebben a körben is előjött, és amit egyetlen kapu sem fog meg, mert nyelvtanilag hibátlan promptot hagy maga után. |
