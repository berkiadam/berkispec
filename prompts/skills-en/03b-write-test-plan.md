...
| The category dictionary | The `<field:f_test_category>` values of the table are **subsets** of the `**<field:f_test_categories>:**` dictionary of `conventions.md` (KT2) — the mechanical gate measures it. |
...
- **`docs-generated/test-description.md` — the test inventory (LD9), the CURRENT TRUTH about the EXISTING tests.** If it exists, **read it** — this is the only place where it is visible in one place which tests already exist in the project and what they prove. Use it for two things: (a) for **selecting the regression round** of `<sec:regression_impact>` (which existing `TL-NNN` is affected by the change of the cycle), and (b) for **avoiding duplication** — do not prescribe a `TS-NN` scenario that an existing `TL-NNN` item already covers; if you **extend** an existing test, state which `TL-NNN` it is about. **You do NOT write this file** — its owner is `08-doc-sync` (LD1); the inventory is updated at the end of the cycle, in the doc-sync. **Guard:** if the file does not exist, do not stop and do not create it — state it in one sentence.
...
|---|---|---|---|---|---|---|---|---|
| unit | gyors | — | `<the verbatim command, with a machine reporter>` | `junit.xml` | junit | — | local | <status:phase_implement>, <status:phase_validate> |
| integration | gyors | — | `<command>` | `<file>` | junit | — | local | <status:phase_implement>, <status:phase_validate> |
...
**Rules for filling it in:**
- **The category (KT2):** the value of the cell comes from the `**<field:f_test_categories>:**` dictionary of the `## <sec:cv_test_execution>` section of `conventions.md` — **do not invent a new name**. The project-level table collects into the `test-runs/<category>/` tree under these same names, so an `e2e` ↔ `rest-e2e` drift would collect results about the same thing under two names. If a genuinely new category was created, extending the dictionary of `conventions.md` is **part of the cycle** (the "Gate configuration moves together" rule). `analyze-gate-check.py` measures this.
...
- **🔴 <field:f_phase> — which PHASE runs it (PH1). MANDATORY and EXPLICIT: nothing is implicit.** The cell may be a **comma-separated list** of **four** values, in any combination: `<status:phase_implement>` (the dev loop of 06), `<status:phase_validate>` (07-validate), `<status:phase_post_merge>` (the post-merge `VP2` round), `<status:phase_dev_test>` (the `VP3` round of `bs-dev-test`). **An empty cell is an ERROR, not a default**, and the `both` value is gone — with four phases it would be ambiguous anyway. `run-tests.py` filters with the `--phase` switch: `06` calls it with `--phase <status:phase_implement>`, `07` with `--phase <status:phase_validate>`, the merge branch with `--phase <status:phase_post_merge>`, and `bs-dev-test` with `--phase <status:phase_dev_test>`.
  ...
  - **When `<status:phase_post_merge>`:** what has to be measured on the code **merged** with the fresh main branch, before the cycle branch is merged in (`VP2`). **In centralized SDD this round runs on the test node of the CI, so it typically has to be fully containerizable** — do not mark a category here that needs an environment brought up by hand on the developer's machine.
  - **When `<status:phase_dev_test>`:** the real e2e round running against the deployed, integrated dev environment (`VP3`, `bs-dev-test`) — it only makes sense on the centralized path.
  - **🔴 A test proving a `DoD-NN` can never be `<status:phase_implement>`-only.** With `dod-check.py`, `07` joins evidence from the **validation round**: whatever ran only in 06 leaves the DoD without evidence, and the item stays at `?`. If a category is needed for the DoD, the list has to contain `<status:phase_validate>` as well.
  - **Why the cell became mandatory:** silence used to mean "runs everywhere". With the introduction of `<status:phase_post_merge>` that became dangerous: an unmarked test would have fallen silently into the post-merge round, and with it the requirement of a containerized environment would have been loaded onto **every** test. In old plans an empty cell / `both` is still accepted **on read** with the old meaning and a WARN — **a new plan can no longer write it** (the closing gate stops with a FAIL in `--plan-only` mode).
- **An empty cell:** `—`.
