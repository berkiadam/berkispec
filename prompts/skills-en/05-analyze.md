...
| A spec test case has not been converted into a `TS-NN` (`TS7`), or the data sheet of the test artifact is missing (`TA1`) | **03** | from the plan alone the test cannot be written and run |
| The purpose of the planned change is missing (`WY1`) | **03** | without a purpose it cannot be decided when the change is done, nor whether a different solution is acceptable |
| A `TS-NN` / run category has no owner in tasks.md (`TT1`) | **04** | the coverage chain used to skip the tests: no task belonged to the scenarios of the plan |
| Two `[CHECK]`s write into the same file (`T6`) | **04** | the second run overwrites the evidence of the first |
| A missing test reference or a merged run (`TI2`, `TX1`) | **04** | the tick is not a claim bound to an identifier: it does not come out which plan test case ran |
| A test identifier not in `TC-NN` form or with a gap (`TI1`) | **03** | `tasks.md` and the log of 07 refer to it — renumbering silently breaks the chain |
...
> **This is not just prose self-review — both closing branches of "Status handling" must run `analyze-gate-check.py --report-only` (RC1–RC3, see there).** Experience shows that the very model that has an incentive for a terse closing will not reliably police this list on itself — the closing gate makes it mechanical: not a single `analyze-task.md` item may vanish from the report without a word.
...
1.a **Closing gate (RC1–RC3) — mechanical, not self-review.** Run:
   ```bash
   python3 <platform-scripts-dir>/analyze-gate-check.py specs/cycle-NN-<cycle-name> --report-only
   ```
   This checks whether `analyze-report.md` **kept every item of `analyze-task.md` verbatim** (whether the diagnosis shrank into a summary), and whether the remaining items carry all four mandatory fields (`The contradiction` / `Why it blocks` / `How it would be correct` / `State`), in a closed state. **If it returns a `Must Fix`, do not proceed to step 2** — restore the missing items/fields in the report and rerun until clean. This is not a new analyze iteration (does not consume `X`): the report is your own writing, not the fixers'.
...
1.a **Closing gate (RC1–RC2) — mechanical, not self-review.** Run the same command as on the PASS branch:
   ```bash
   python3 <platform-scripts-dir>/analyze-gate-check.py specs/cycle-NN-<cycle-name> --report-only
   ```
   On `FAIL` the items left open (`[ ]`) are deliberately left open (RC3 only blocks on `PASS`) — but item preservation (RC1) and the mandatory fields (RC2) still count here: the `FAIL` report also has to stay a human-readable, fixable diagnosis for the continuation.
