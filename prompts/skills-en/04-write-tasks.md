...
- **Verification task:** the `[CHECK]` prefix, mandatory at the end of every logical group — it contains a concrete command from the `<sec:verification_strategy>` section of the plan (e.g. `npm test`, `npm run typecheck`). The file path may be omitted.
  - **🔴 The command must be runnable VERBATIM (T5).** No unfilled placeholder may be left in it — `{round}`, `{n}`, `<round>` — because `06` issues the command unchanged: it either fails, or writes to a made-up path, and the evidence will not be where `07` looks for it. If the command needs a path, write it out in full (`specs/cycle-NN-<cycle-name>/test-report/implement/unit.log`); if the location is only known to a later phase (e.g. the round number of `07`), then it is **not a `[CHECK]` task** but a row of the plan's `<sec:machine_run_table>`. The mechanical gate catches this with the `T5` code.
