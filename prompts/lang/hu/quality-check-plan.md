<!--
  ...
  KÉT horgony él benne: a `TP2-code` a 03a-write-code-plan, a `TP2-test` a
  03b-write-test-plan lezárási kapuja (D7).
...
<!-- ANCHOR:TP2-code -->
[ ] 1. WY1 — CÉL MINDEN BEJEGYZÉSHEZ: a `Tervezett módosítások` MINDEN `[P-…]`
...
[ ] 2. Minden tervezett új állomány (script, tesztfájl, fixture, helper,
       wrapper), amelyet a plan futtatni vagy használni fog, szerepel a
       `Tervezett módosítások` közt ÚJ FÁJLKÉNT — egy futtatandó, de sehol
       nem megtervezett állomány biztos bukás a végrehajtáskor.
[ ] 3. KO1 — KÖRNYEZETI KOORDINÁTÁK: a `Környezeti koordináták` szekció megvan
...
[ ] 4. GC1 — KAPU-KONFIGURÁCIÓ: ha a ciklus kapu-olvasott konvenciót érint
...
[ ] 5. KX3 — CSONKÍTÁS-MENTESSÉG: a spec MINDEN kidolgozott artefaktuma
...
[ ] 6. SCOPE-KAPU (SC1): a `Fordított lefedettség` tábla kitöltve, az első
...
[ ] 7. HORGONY-VERIFIKÁCIÓ: minden konkrét `fájl:hely` / „ez a szimbólum ebben a
       ...
       attribúció nincs.
[ ] 8. ÉRTÉK-JÓZANSÁG: átnéztem a plan MINDEN konkrét értékét — portok
...
[ ] 9. SZEKCIÓ-ID (PID1): minden végrehajtható terv-szekció címe visel egyedi
...
[ ] 4. TS7 — SPEC-TESZTESET → `TS-NN`: a spec MINDEN tesztesete önálló `TS-NN`
...
[ ] 5. TA1 — TESZT-ARTEFAKTUM ADATLAP: minden `#### <tesztfájl path>` fejléc alatt
...
[ ] 6. TD7 — MINDEN TESZTESET MEGMONDJA, MIT ELLENŐRIZ: a `TS-NN` blokkok
       ...
       cél.
[ ] 7. TI1 — TESZT-AZONOSÍTÓK: a forgatókönyvek `TS-01`-től, a teszt-tábla
...
[ ] 8. TS8 — `.http` ALAK: minden REST-lépést tartalmazó `TS-NN` blokkban van
...
[ ] 11. Nincs a teszt-szekcióban hivatkozás a lépések HELYÉN: „a cycle-XX mintájára",
       ...
       fejlécekkel".
[ ] 12. Minden teszteset futtatható belépési pontja (script/tesztfájl) létezik a
       repóban VAGY szerepel a `Tervezett módosítások` közt új fájlként, és az
       `Ellenőrzési stratégia` parancsa pontosan ezt hívja.
[ ] 13. KÖRNYEZET-FELKÉSZÍTÉS (TP3): a plan tartalmazza szó szerinti parancsként a
...
[ ] 14. Amit a teszt futtatása igényel, de sem a `test-conventions.md`-ben, sem ebben
