...
- _`Test manager`: `none` (alap) · `testdino` · `reportportal` **(under development)** · `qase` **(under development)** · `command` — melyik adapter fut. `none` esetén a többi mező elhagyható._
  - _**⚠ Adapter-érettség (2026-09-22).** A `testdino` ág **éles fiókkal, végponttól végpontig mérve** (füstteszt: `fixtures/testdino-smoke/`). A `reportportal` és a `qase` ág **UNDER DEVELOPMENT**: a kód megvan és a `TM6` szerződést tartja, de **valódi szolgáltató ellen soha nem futott** (a megvalósításkor nem volt hozzájuk fiók). Használhatod, de a feltöltő hívást a szolgáltató **aktuális** CLI-jével validálni kell — addig a `command` ág a becsületes választás, és **ne tedd kapuvá** (`Test manager kötelező: igen`). A `test-manager.py` minden módban kiírja a figyelmeztetést._
