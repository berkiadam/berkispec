...
- _`Test manager`: `none` (alap) · `testdino` · `reportportal` · `qase` · `command` — melyik adapter fut. `none` esetén a többi mező elhagyható. **⚠ Kipróbáltság (2026-09-22):** a `testdino` ág **élesben mérve** (füstteszt valódi fiókkal); a `reportportal` és a `qase` ág **KIPRÓBÁLATLAN** — a megvalósításkor nem volt hozzájuk fiók, a feltöltő hívásukat a szolgáltató aktuális CLI-jével validálni kell. Amíg ez nem történt meg, a `command` ág a becsületes választás (a szolgáltató saját CLI-jével), és a `test-manager.py --mode selftest` is kiírja a figyelmeztetést._
