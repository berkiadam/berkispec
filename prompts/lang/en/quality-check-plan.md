<!--
  ...
  It holds TWO anchors: `TP2-code` is the closing gate of 03a-write-code-plan,
  `TP2-test` that of 03b-write-test-plan (D7).
...
<!-- ANCHOR:TP2-code -->
[ ] 1. WY1 — A PURPOSE FOR EVERY ENTRY: EVERY `[P-…]` section of the
...
[ ] 2. Every planned new artifact (script, test file, fixture, helper,
       wrapper) that the plan is going to run or use appears among the
       `Planned changes` AS A NEW FILE — an artifact that is run but
       designed nowhere is a certain failure at execution time.
[ ] 3. KO1 — ENVIRONMENT COORDINATES: the `Environment coordinates` section
...
[ ] 4. GC1 — GATE CONFIGURATION: if the cycle touches a gate-read convention
...
[ ] 5. KX3 — NO TRUNCATION: EVERY elaborated artifact of the spec (OpenAPI/JSON/
...
[ ] 6. SCOPE GATE (SC1): the `Reverse coverage` table is filled in, with a `[P-…]`
       ...
       spec source.
[ ] 7. ANCHOR VERIFICATION: I CONFIRMED with a Grep/Read command every concrete
...
[ ] 8. VALUE SANITY: I went through EVERY concrete value of the plan — ports
...
[ ] 9. SECTION ID (PID1): the title of every executable plan section bears a unique
...
[ ] 4. TS7 — SPEC TEST CASE → `TS-NN`: EVERY test case of the spec has been
...
[ ] 5. TA1 — TEST ARTIFACT DATA SHEET: under every `#### <test file path>`
       ...
       `TC-ID` / `TS-NN`) line.
[ ] 6. TD7 — EVERY TEST CASE SAYS WHAT IT VERIFIES: the
...
[ ] 7. TI1 — TEST IDENTIFIERS: the scenarios run from `TS-01`, the cases of
...
[ ] 8. TS8 — `.http` FORM: every `TS-NN` block containing a REST step also has
...
[ ] 14. Whatever running the test requires but is present neither in
