...
- _`Test manager`: `none` (default) · `testdino` · `reportportal` **(under development)** · `qase` **(under development)** · `command` — which adapter runs. With `none` the other fields may be left out._
  - _**⚠ Adapter maturity (2026-09-22).** The `testdino` branch was **measured end to end with a real account** (smoke test: `fixtures/testdino-smoke/`). The `reportportal` and `qase` branches are **UNDER DEVELOPMENT**: the code is there and it keeps the `TM6` contract, but it has **never run against a real provider** (there was no account for them at implementation time). You may use them, but the upload call has to be validated against the **current** CLI of the provider — until then the `command` branch is the honest choice, and **do not make it a gate** (`Test manager required: yes`). `test-manager.py` prints the warning in every mode._
...
*"Do you use an **external test manager** (TestDino, ReportPortal, Qase, TestRail, Xray …) where test results are collected across cycles? This is **optional and off by default** (`none`) — the evidence of the cycle remains the committed report, the test manager adds the trend and flaky data. If yes: which provider, and does its client go into the **reporter chain** of the test runner (`reporter`), or does a command **push the finished `junit.xml` afterwards** (`import`)? ⚠ Of the four adapters, `testdino` has been tried out with a real account; `reportportal` and `qase` are **under development** (they have not yet run against a real provider) — for those the `command` branch is the safe choice."*
