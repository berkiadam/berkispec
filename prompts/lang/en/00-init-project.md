...
- _`Test manager`: `none` (default) · `testdino` · `reportportal` · `qase` · `command` — which adapter runs. With `none` the other fields may be left out. **⚠ How far they were tried out (2026-09-22):** the `testdino` branch was **measured live** (a smoke test with a real account); the `reportportal` and `qase` branches are **UNTRIED** — there was no account for them at implementation time, so their upload call has to be validated against the current CLI of the provider. Until that has happened, the `command` branch is the honest choice (with the provider's own CLI), and `test-manager.py --mode selftest` prints the warning as well._
