...
- **a teljes `DoD-NN → [P-…] → task` lefedettségi lánc (C1/C2), a `Spec-lefedettség` TP1-teljessége (C3), a `Konfiguráció-életút` üres cellái (C4), a task-határon átnyúló shell-változó (C5)** — AG4;
...
3. **Alulspecifikáció** — hiányzó elfogadási feltétel; a spec valós implementációt ír elő, de a plan csak mockot/szimulációt tervez; taskhoz nem rendelhető konkrét plan-szekció.
   - **Ami a kapuban már eldőlt (ne ismételd):** a `DoD-NN` kimaradása a `Spec-lefedettség` táblából (TP1 → `C3`), a `Konfiguráció-életút` üres cellái (KF1 → `C4`), a `Fordított lefedettség` sorainak `[P-…]` azonosítója (`S3`), és a kötelező táblák puszta megléte (`S1`/`S2`).
