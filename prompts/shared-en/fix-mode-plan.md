...
> **🔴 Completeness accounting — no item may disappear silently.** **Every** item of the `<status:must_fix>` list you received must appear in your summary on **exactly one** branch: either (a) — you fixed it, and you describe how —, or (b) — you raised it as a `Qnn` question, because it requires a genuine decision. There is no third option: "I could not get to it", "in the next round" and silent omission are **not** acceptable. If you can neither fix an item nor turn it into a question, **say so in one line, with its identifier and the reason** — the orchestrator has a separate branch for that. Always refer to the items by the **identifier you received** (`AF-NN` / `AX-NN`), never paraphrase their text: the survival rule of the loop builds on this.
