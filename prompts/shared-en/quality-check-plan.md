...
- **🔴 Does every integration and E2E test contain the step-by-step call chain?** — Go through it **test case by test case**: is the HTTP verb, the full endpoint, the headers (including the type of the `Authorization`), the **concrete request body**, the expected status and the key response fields present at every step? For browser E2E, the user interaction **and** the network call belonging to it **and** the visible result? **If there is only a reference to an earlier cycle, to an existing test file or to the sequence diagram of the spec anywhere, the quality check FAILS** — spell out the steps, then run it again.
