...
- **Empty test body (TB1) — a decidable question:** Is there any **new or modified test function** in the diff whose body contains **no assertion bound to the response or the state of the system** (only `assert True`, `pass`, a constant comparison, or exclusively a check of the mock's own return value)? If yes, that is **<status:must_fix>** — the test is green but proves nothing. This point is **to be decided, not weighed**: either such an assertion is in the body or it is not. _(Why it is hard: in a real cycle `assert True` skeletons got into the test file — the suite reported `X passed`, the DoD got its evidence, and validation closed with `PASS` without anything having been checked.)_
