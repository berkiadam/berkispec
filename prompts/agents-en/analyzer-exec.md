...
```md
## Must Fix
- [ ] **AX-NN** — <category (6.x)> — <description: what contradicts what, BOTH sides with their own `file:location`> → target phase: <phase> (`file:location`)
      **why it blocks:** <one sentence: what can break in the implementation if it stays like this>
      **how it would be correct:** <one or two sentences: in what state it would be executable — or, if it takes a real decision, the question to be decided>
...
- **Every `<status:must_fix>` item mandatorily gets a `AX-NN` identifier** (`AX-01`, `AX-02`, …). The identifier is **stable**: from the 2nd run onward **do not renumber** the items — the ones still open keep their number, new ones continue at the end of the sequence, and in the `Previous round's Must Fix items` block you refer to them with the same identifier. The orchestrator's **survival rule** builds on this (if the same identifier survives two consecutive iterations, that signals a **decision**, not a fixable defect) — with paraphrased text it does not work.
  > The prefix is **`AX`**, not `AF` — the orchestrator merges your list with that of `analyzer`, and the identifiers must not collide.
