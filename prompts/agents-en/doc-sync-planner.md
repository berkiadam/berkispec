...
6. **Technical-contract and environment preservation (DS23/DS25).** You must carry over `spec.md`'s worked-out technical contracts (config field table, log/event JSON schema, error-code table) and the non-secret values of `plan.md`'s `Environment coordinates` (KO1) table (URL, port, path, dev test user) **verbatim, without truncation** — do not replace them with an "the spec describes it in detail" summary sentence. The surgical-patch principle (rule 2) still applies here — you only write the section that CHANGES — but the CONTENT of that section must match the spec's level of detail, not be a condensed version of it. The target does not have to be `architecture.md`: a component-specific doc (e.g. `redis-usage.md`) is also a good home if its header scope covers it. Never write a secret (password/secret/token) into the replacement text — put a pointer there instead (the TC5 principle).
