...
2. **Input:** the unfinished tasks of the **active fixing section** — in the base case (the loop of `07`) those of the `## <sec:validation_fixes>` section of `tasks.md` (the concrete failed tests / Sonar failures / unfulfilled DoD items), with the prerequisite references of the section (`validation-report.md`, and `sonar-report.md` if there is one) + the current state of `tasks.md`. **The loop can also be started by the merge branch of the cycle end** (`auto-fix-loop`, `L13-D10`) — in that case the active section is `## <sec:post_merge_fixes>` and the prerequisite is the report in `test-report/post-merge/` or `test-report/dev-test/`. **The caller always says which section you work on** — do not assume `validation_fixes`.
