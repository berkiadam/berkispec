...
6. **Technikai szerződés- és környezet-mentesség (DS23/DS25).** A `spec.md` kidolgozott technikai szerződéseit (config-mezőtábla, log/esemény-JSON-séma, hibakód-tábla) és a `plan.md` `Környezeti koordináták` (KO1) táblájának nem-titok értékeit (URL, port, elérési út, dev teszt-user) **szó szerint, csonkítás nélkül** kell átvinned — ne "a spec részletesen leírja" jellegű összefoglaló mondattal helyettesítsd. A sebészi-patch elv (2. pont) itt is érvényes — csak a VÁLTOZÓ szekciót írod —, de a szekció TARTALMA a specével egyenértékű részletességű legyen, ne annak zanzája. A cél nem feltétlenül az `architecture.md`: egy komponens-specifikus doksi (pl. `redis-usage.md`) is jó otthon, ha a fejléc-scope-ja lefedi. Titkot (jelszó/secret/token) **soha ne írj a csereszövegbe** — pointer kerül helyette (TC5 elve).
