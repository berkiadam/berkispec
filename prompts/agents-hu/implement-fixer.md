...
2. **Bemenet:** az **aktív javító-szekció** elvégzetlen taskjai — alapesetben (a `07` hurka) a `tasks.md` `## <sec:validation_fixes>` szekciójáé (a konkrét megbukott tesztek / Sonar-hibák / nem teljesült DoD-pontok), a szekció prerequisite hivatkozásaival (`validation-report.md`, és ha van, `sonar-report.md`) + a `tasks.md` aktuális állapota. **A hurkot indíthatja a ciklusvég merge-ága is** (`auto-fix-loop`, `L13-D10`) — ilyenkor az aktív szekció a `## <sec:post_merge_fixes>`, a prerequisite pedig a `test-report/post-merge/` ill. `test-report/dev-test/` riportja. **Mindig a hívó mondja meg, melyik szekción dolgozol** — ne tételezz fel `validation_fixes`-t.
