---
name: bs-quick-flow
description: "berkispec - quick-flow. Egyszerűsített, háromfázisú SDD flow (spec-plan.md → tasks.md → implementáció) KIS, jól körülhatárolt feladatokhoz, amelyek 3-4 lépésben megoldhatók — konfiguráció, egyszerűbb script, kisebb javítás, lokális módosítás. Nagyobb kódírást, több komponenst érintő funkciót vagy összetett tervezést igénylő feladathoz NE ezt használd — javasold a teljes berki spec folyamatot (01-add-cycles)."
output:
  - "specs/cycle-NN_quick-flow_<cycle-name>/spec-plan.md státusz: <status:ready_for_tasks>"
  - "specs/cycle-NN_quick-flow_<cycle-name>/tasks.md státusz: <status:ready_for_implement> → <status:done>"
subagents:
...
2. **Ciklusmappa.** Határozd meg a következő szabad ciklusszámot a **BQ2** formulával, javasolj nevet, kérj jóváhagyást, majd hozd létre: `specs/cycle-NN_quick-flow_<cycle-name>/`.
3. **1. fázis — `spec-plan.md`.** Írd meg a specifikációt (cél, paraméterek, **technikai vázlat/megközelítés**, tesztstratégia **cél-környezettel**, README-terv), `<field:f_status>: <status:draft>` fejléccel. A végén futtass **konzisztencia-ellenőrzést** és az **RP1 útvonal-kaput**. **⛔ ÁLLJ MEG**, és várd meg a Felhasználó explicit jóváhagyását.
...
*   **Ciklusok (Cycles):** minden önálló feladat, funkció vagy fejlesztési szakasz dedikált mappában él, a `cycle-NN_quick-flow_<cycle-name>` séma szerint (pl. `cycle-01_quick-flow_database-config`, `cycle-02_quick-flow_logging-fix`).
*   **Dokumentumvezérelt fejlesztés:** kódot írni vagy módosítani tilos, amíg az 1. és a 2. fázis le nem zárult. A projekt fő `README.md`-jének naprakészen tartása **nem opcionális**: mindig a `spec-plan.md` és a `tasks.md` része.
*   **Két artefaktum, két státusz:** a ciklus mappájában **pontosan két** tervezési dokumentum él — `spec-plan.md` és `tasks.md` —, és mindkettő fejléce hordoz egy `<field:f_status>` mezőt (QF2). Nincs `plan.md`: annak a szerepét a `spec-plan.md` technikai vázlata veszi át — **a fájlnév is ezt mondja ki** (QF21), a spec és a terv itt egyetlen dokumentumba van összevonva.
*   **A dokumentáció nyelve és hangja:** a ciklus-dokumentumok a fájl elején álló **A kimenet nyelve** blokkot követik; a kódban használt azonosítók, kapcsolók és technikai kifejezések angolul maradnak. **Az artefaktum az implementálónak szól, nem neked (AV1):** a skill-szöveget — imperatívuszokat („tilos…", „kötelező ellenőrizned…"), `🔴` jelölést — ne másold át a `spec-plan.md`/`tasks.md`-be; írd le **döntésként**, leíró hangnemben.

---

<!-- INCLUDE:shared/path-format.md -->

> **Ez a flow EGYETLEN kötelező determinisztikus kapuja (QF11).** Az `--paths-only` a ciklus mappájának `spec-plan.md` / `tasks.md` párjára fut, tehát az 1. és a 2. fázis lezárása előtt is. A többi kapu-script (`analyze-gate-check.py` teljes mód, `run-tests.py`, `dod-check.py`, `report-gate-check.py`) a teljes flow-hoz tartozik, és **itt nem fut** — ha ezekre lenne szükséged, az a túlnövés jele.

---

<!-- INCLUDE:shared/dereferencing.md -->

> **Ebben a flow-ban ez még szigorúbb (KX2):** nincs `plan.md`, tehát a `spec-plan.md` az **egyetlen** végrehajtási igazság — amit hivatkozásként hagysz, azt a 3. fázisban senki nem oldja fel helyetted. A `<sec:environment_coords>` helyét a `spec-plan.md` technikai vázlata és tesztstratégiája veszi át, a feloldott értékekkel (parancsok, URL-ek, payload, koordináták).

---

<!-- INCLUDE:shared/conventions-change.md -->

> **A négy feltétel leképezése erre a flow-ra (GC1):** a 2. feltétel (a plan tervezi) helyett a `spec-plan.md` **technikai vázlata** rögzíti a `conventions.md` érintett szekciójának új tartalmát; a 4. feltétel (a `07` teljes köre újra fut) helyett a 3. fázis **tesztje** fut a frissített `conventions.md`-vel. Az 1. és a 3. feltétel változatlan: explicit döntés a `spec-plan.md`-ben, külön task a `tasks.md`-ben. Ez a flow tipikus feladata (konfiguráció, port, teszt-parancs) épp az, amit a kapuk a `conventions.md`-ből olvasnak — a frissítése tehát a ciklus része, nem a következő ciklus adóssága.
...
1. **Git környezet előkészítése — a `conventions.md`-ből (QF1):** a ciklus indítása előtt olvasd be a projekt gyökerében a `conventions.md` `## <sec:cv_git_conventions>` szekcióját, és **onnan** vedd a paramétereket — ne drótozz be ágnevet vagy commit-formátumot:
   ...
   * **<field:f_main_branch>** (alapból `main`): ha ezen az ágon állsz, kérd be a Felhasználótól a ciklus rövid nevét (és amit a konvenció megkövetel — pl. Jira-azonosítót), majd hozd létre a feature ágat a **<field:f_branch_naming>** szerint (alapból `feature/cycle-NN_quick-flow_<cycle-name>`), és válts rá.
...
2. **Cél megadása és interjú (grill = addig kérdezel, amíg minden tiszta):** A Felhasználó leírja, miről szól a ciklus (milyen funkciót, javítást vagy módosítást kell megvalósítani). Kérdezz addig, amíg minden szükséges információ a kezedben van a specifikáció megírásához. Brainstorm-átvételnél a belépő szekció szerint járj el (QF16).
   * **Flow-méret ellenőrzés (kötelező):** Az interjú alatt végig mérlegeld, hogy a feladat tényleg illik-e az egyszerűsített flow-ra (lásd a „Mikor ezt a flow-t…" szekciót). Ha túlnő rajta (nagyobb kódírás, több komponens, összetett tervezés), **állj meg és javasold a teljes berki spec folyamatot** (`/bs-add-cycles`), mielőtt belekezdenél a `spec-plan.md`-be.
...
4. **Névjavaslat:** A leírás és az interjú alapján javasolj nevet az új ciklusnak (kisbetűs, kötőjeles, pl. `add-health-check` vagy `fix-tls-handshake`), és vele a teljes mappanevet (pl. `cycle-03_quick-flow_add-health-check`). **A mappanév mindig a `cycle-NN_quick-flow_<cycle-name>` sémát kövesse (QF22):** a `quick-flow` tag alulvonások közé kerül, a ciklusszám és a név elválasztója marad a kötőjel — így a mappanévből (és a belőle képzett ágnévből) ránézésre látszik, hogy a ciklus az egyszerűsített úton készült, `plan.md` és analyze/validate/doc-sync fázis nélkül. Az átnevezés előtt indult quick-flow ciklusok mappaneve marad, ami volt.
5. **Jóváhagyás:** A Felhasználó jóváhagyja vagy módosítja a javasolt nevet és sorszámot.
6. **Inicializálás:** A jóváhagyás után hozd létre az új ciklusmappát a `specs/` alatt (pl. `specs/cycle-03_quick-flow_add-health-check/`), és kezdd el benne a `spec-plan.md`-t (1. fázis). A `spec-plan.md` megírásához kötelezően használd fel az interjú során gyűjtött teljes kontextust.
...
```mermaid
graph TD
    A[1. Fázis: spec-plan.md / Piszkozat] -->|Jóváhagyás → Task írásra kész + commit| B[2. Fázis: tasks.md / Piszkozat]
...
### 1. Fázis: Specifikáció (`spec-plan.md`)
Ebben a fázisban tisztázzuk a követelményeket és rögzítjük a pontos technikai tervet.
*   **Lépés:** Hozz létre egy `spec-plan.md` fájlt az aktuális `cycle-NN_quick-flow_<cycle-name>` mappában, a fejlécében a státusz-mezővel (QF2):
...
*   **Tartalom:** 
    ...
    *   **Megközelítés / technikai vázlat (a `plan.md`-t helyettesítő állványzat — kötelező):** Mielőtt a `tasks.md`-re lépnél, a `spec-plan.md`-ben rögzítsd a megvalósítás technikai HOGYAN-ját — ez adja a gyengébb/olcsóbb modellnek azt az állványzatot, amit a teljes flow-ban a külön `plan.md` biztosítana. Tartsd tömören (jellemzően 3-6 pont), és maradj szigorúan a spec scope-ján belül (ne tervezz olyat, ami a célból nem következik):
    ...
    *   **🔴 Merge-tesztek — kötelező szekció a `spec-plan.md` végén (QF23):** `## <sec:merge_tests>` + `**Post-merge test categories:** <kategóriák a `conventions.md` `## <sec:cv_test_execution>` szótárából>`. A merge-ág ezt **szűri** a projekt-szintű táblán (`run-tests.py --table-source conventions`) — a *hogyan fut* ott marad, itt csak **kiválasztasz**. **Üresen nem hagyható**: ha tényleg nincs mit futtatni, írd ki, hogy `nincs` + fél mondat indok (a hallgatás megkülönböztethetetlenné tenné a „nincs mit tesztelni" és az „elfelejtette" esetet). `SDD mode: centralized` mellett a hiánya `bs-merge` kapu-hiba.
...
*   **Szabály (Kritikus):**
    ...
    *   **⛔ ÁLLJ MEG a fázis végén.** A 2. fázist (a `tasks.md`-t) **csak akkor kezdd el, ha a Felhasználó kifejezetten (explicit módon) jóváhagyta** a `spec-plan.md`-t. Jóváhagyás nélkül ne lépj tovább.
    *   **Fázis-zárás (státusz + commit, egyetlen lépéspár):** a jóváhagyás elhangzása után azonnal, még a 2. fázis megkezdése előtt (a) írd át a `spec-plan.md` `<field:f_status>` mezőjét `<status:ready_for_tasks>`-ra, majd (b) commitolj: `git add specs/cycle-NN_quick-flow_<cycle-name>/`, és a `conventions.md` szerinti üzenettel `git commit`. A kettő között ne kérdezz, ne várj, ne kezdj más munkába. **Ellenőrizd determinisztikusan, ne „érzésre":** `git log -1 --oneline && git status --short specs/cycle-NN_quick-flow_<cycle-name>/` — az első sorban a most készült commit álljon, a `git status` kimenete a ciklus mappájára **üres** legyen; ha nem, javítsd és futtasd újra (legfeljebb 2 próbálkozás). A válaszodban írd ki a commit azonosítóját. Külön engedélyt ne kérj rá: a fázis jóváhagyása magában foglalja. No-VCS ágon a commit kimarad, a státuszírás nem. _(A közös eljárás forrása a `phase-commit.md` blokk; a commit-üzenet formátuma viszont itt a `conventions.md`-ből jön, nem a `cycle-NN: <fázis-tag>` alakból.)_
...
*   **Belépő fázis-kapu (QF4):** olvasd be a `spec-plan.md` fejlécének `<field:f_status>` mezőjét (egy még az átnevezés előtt indult ciklusban a fájl neve `spec.md` — akkor azt olvasd, és menet közben ne nevezd át). Ha az értéke **nem** `<status:ready_for_tasks>`, **STOP** — a spec nincs jóváhagyva (vagy a jóváhagyás nem lett commitolva). Ne a beszélgetés emlékére hagyatkozz: `/clear` vagy megszakadás után a státusz-mező az egyetlen, ami túléli.
*   **Lépés:** Hozz létre egy `tasks.md` fájlt az aktuális `cycle-NN_quick-flow_<cycle-name>` mappában, `**<field:f_status>:** <status:draft>` fejléccel.
*   **Tartalom:**
    *   **A „Megközelítés / technikai vázlat" a kiindulópont:** a task-bontás a `spec-plan.md`-ben rögzített technikai vázlatra (érintett fájlok, kulcs-elemek, végrehajtási sorrend) épüljön — a lépések sorrendje kövesse a vázlat végrehajtási sorrendjét. Ha bontás közben a vázlat hiányosnak vagy tévesnek bizonyul, az **spec-hiány**: lépj vissza az 1. fázisba és egészítsd ki (újra-jóváhagyással), ne a `tasks.md`-ben pótold csendben.
...
*   **Fázis-visszalépés (spec-hiba esetén):** ha menet közben kiderül, hogy a `spec-plan.md` hiányos vagy téves, **tilos csendben eltérned** tőle. Ehelyett:
    1. Állj meg.
    2. Lépj vissza az 1. fázisba, és frissítsd a `spec-plan.md`-t (és ha kell, a `tasks.md`-t) — a `spec-plan.md` státusza ilyenkor visszaáll `<status:draft>`-ra.
...
*   **Befejezési feltétel / Ciklus Lezárása:** Az implementáció és a teljes ciklus **kizárólag akkor tekinthető késznek és lezártnak**, ha:
    ...
    5. **⛔ A merge-ág is lefutott (QF24).** Ennek a flow-nak a hatóköre a saját dokumentumai frissítéséig tart — **utána KÖTELEZŐ** a `conventions.md` `## <sec:cv_review_and_merge>` szekciója szerinti ág: `PR submission: no` → `/bs-review-and-merge`, `yes` → `/bs-create-pr` → `/bs-review` → `/bs-merge`. Belépő kapuként ott a `tasks.md` státusza áll a hiányzó `plan.md` helyett (a `QF8` kétbelépős mintája). No-VCS projektben az egész ág kimarad.
...
| `analyzer` (2. fázis, read-only) | több követelményes, könnyen kicsúszó task-listánál; `spec-plan.md` ↔ `tasks.md` konzisztencia-diagnózist ad (lefedettségi rés, kétértelműség, alulspecifikáció) | hatókör-paramétert **nem** adsz (a prompt dokumentált degradációs ága mind az öt kategóriát viszi), szelet-fájl nincs; a bemenet a `spec-plan.md` + `tasks.md` **pár** — a `plan.md`-re hivatkozó bemeneti pontja **üres**, a helyét a `spec-plan.md` technikai vázlata veszi át, és lefedettségi mátrix sincs |
| `reviewer` (3. fázis, záró commit előtt, read-only) | nem triviális kódváltozásnál, a commit előtti minőségi kapuként; diff-review konvenciókra, scope-ra, hibakezelésre, spec-megfelelésre | a kötelező `plan.md` helyett a `spec-plan.md` **technikai vázlata** + tesztstratégiája; a kimenet `specs/cycle-NN_quick-flow_<cycle-name>/code-review.md` (a ciklus gyökerében, `test-report/` almappa nélkül); az `MF-NN` azonosítók és az RV-INC inkrementális írás **megmarad**, de **önjavító hurok, per-item számláló és `review-fixer` nincs** — a `<status:must_fix>` tételeket a lezárás előtt **inline** javítod, a `<status:suggestion>`-öket jelzed |
...
| Spec-hiba | 3. fázis | vissza az 1. fázisba, `spec-plan.md` javítása, újra-jóváhagyás |
