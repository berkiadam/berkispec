...
| Kategória-szótár | A tábla `<field:f_test_category>` értékei a `conventions.md` `**<field:f_test_categories>:**` szótárának **részhalmazai** (KT2) — a mechanikus kapu méri. |
...
- **`docs-generated/test-description.md` — a teszt-leltár (LD9), CURRENT TRUTH a MEGLÉVŐ tesztekről.** Ha létezik, **olvasd be** — ez az egyetlen hely, ahol egy helyen látszik, milyen tesztek vannak már a projektben és mit bizonyítanak. Két dologra használd: (a) a `<sec:regression_impact>` **regressziós kör kiválasztásához** (melyik meglévő `TL-NNN` érintett a ciklus változása által), és (b) a **duplikáció elkerüléséhez** — ne írj elő olyan `TS-NN` forgatókönyvet, amit egy meglévő `TL-NNN` tétel már lefed; ha egy meglévő tesztet **kiterjesztesz**, mondd ki, melyik `TL-NNN`-ről van szó. **A fájlt te NEM írod** — a gazdája a `08-doc-sync` (LD1); a leltár frissítése a ciklus végén, a doc-syncben történik. **Guard:** ha a fájl nem létezik, ne állj meg és ne hozd létre — egy mondatban jelezd.
...
**Kitöltési szabályok:**
- **Kategória (KT2):** a cella értéke a `conventions.md` `## <sec:cv_test_execution>` szekciójának `**<field:f_test_categories>:**` szótárából való — **ne találj ki új nevet**. A projekt-szintű tábla ugyanezekkel a nevekkel gyűjt a `test-runs/<kategória>/` fába, tehát egy `e2e` ↔ `rest-e2e` elcsúszás két néven gyűjtene eredményt ugyanarról. Ha valóban új kategória keletkezett, a `conventions.md` szótárának bővítése **a ciklus része** (a „Kapu-konfiguráció együtt mozog” szabály). Az `analyze-gate-check.py` ezt méri.
