...
| Élő riport (AR1) | Az `analyze-report.md` **a legelső diagnózis után azonnal** elkészül `IN_PROGRESS` státusszal, emberi nyelvű pipálólistával (*mi a baj · miért blokkol · célfázis · állapot*) — nem a hurok végén. A hurok minden lépése frissíti, így a felhasználó látja, min dolgozik éppen a fázis. Lezáráskor **mechanikus záró-kapu** (`analyze-gate-check.py --report-only`, RC1–RC3) ellenőrzi, hogy a lista nem zsugorodott-e összefoglalóvá. |
...
| Spec-teszteset nincs `TS-NN`-né konvertálva (`TS7`) vagy hiányzik a teszt-artefaktum adatlapja (`TA1`) | **03** | a terv alapján a tesztet nem lehet megírni és lefuttatni |
| A tervezett módosítás célja hiányzik (`WY1`) | **03** | cél nélkül nem eldönthető, mikor kész a változtatás, és hogy egy eltérő megoldás jó-e |
| `TS-NN` / futtatási kategória gazdátlan a tasks.md-ben (`TT1`) | **04** | a lefedettségi lánc eddig kihagyta a teszteket: a plan forgatókönyveihez nem tartozott task |
| Két `[CHECK]` ugyanabba a fájlba ír (`T6`) | **04** | a második futás felülírja az első bizonyítékát |
| Teszt-hivatkozás hiánya vagy összevont futtatás (`TI2`, `TX1`) | **04** | a pipa nem azonosítóhoz kötött állítás: nem derül ki, melyik plan-teszteset futott le |
| Nem `TC-NN` alakú vagy hézagos teszt-azonosító (`TI1`) | **03** | a `tasks.md` és a 07 naplója erre hivatkozik — átszámozás némán elrontja a láncot |
...
> **Ez nem csak prózai önellenőrzés — a „Státusz kezelés" mindkét záró ágán kötelező futtatni a `analyze-gate-check.py --report-only` parancsot (RC1–RC3, lásd ott).** A tapasztalat az, hogy épp az a modell, amelyiknek érdeke a tömör lezárás, nem fogja megbízhatóan magán számon kérni ezt a listát — a záró-kapu ezt teszi gépiessé: az `analyze-task.md` egyetlen tétele sem tűnhet el a riportból szó nélkül.
...
1.a **Záró-kapu (RC1–RC3) — mechanikus, nem önellenőrzés.** Futtasd:
   ```bash
   python3 <platform-scripts-mappa>/analyze-gate-check.py specs/cycle-NN-<cycle-name> --report-only
   ```
   Ez ellenőrzi, hogy az `analyze-report.md` **szó szerint megtartotta-e** az `analyze-task.md` minden tételét (a diagnózis nem zsugorodott-e összefoglalóvá), és hogy a megmaradt tételek viselik-e mind a négy kötelező mezőt (`Az ellentmondás` / `Miért blokkol` / `Hogyan lenne helyes` / `Állapot`), lezárt állapotban. **Ha `Must Fix`-et ad, ne folytasd a 2. ponttal** — pótold a hiányzó tételeket/mezőket a riportban, és futtasd újra, amíg tiszta. Ez nem új analyze-iteráció (nem fogyaszt `X`-et): a riport a te írásod, nem a fixereké.
...
1.a **Záró-kapu (RC1–RC2) — mechanikus, nem önellenőrzés.** Futtasd ugyanazt a parancsot, mint a PASS ágon:
   ```bash
   python3 <platform-scripts-mappa>/analyze-gate-check.py specs/cycle-NN-<cycle-name> --report-only
   ```
   `FAIL` mellett a nyitva hagyott (`[ ]`) tételek szándékosan nyitottak maradnak (RC3 csak `PASS`-nál blokkol) — de a tétel-megőrzés (RC1) és a kötelező mezők (RC2) itt is számítanak: a `FAIL` riport is ember által olvasható, javítható diagnózis kell hogy maradjon a folytatáshoz.
