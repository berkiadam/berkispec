...
_**🔴 Minden teszteset kimondja, MIT ellenőriz és MIÉRT (TD7).** Az eset a lépések ELŐTT egy mondatban megmondja, milyen **viselkedést** bizonyít — eldönthető állításként —, és melyik `<sec:definition_of_done>` pontot (`DoD-NN`) vagy kockázatot igazolja. **A cím nem cél:** „Teszteset 3: konkurencia" téma, nem állítás; az állítás: *„öt egyidejű kérésből pontosan egy újítja meg a tokent, a többi a meglévővel szolgál ki"*. Ez a mondat viszi tovább a szándékot a `03`-ba (ott a `<field:f_what_we_test>` sor lesz belőle) és a `07`-be: enélkül egy bukott tesztről nem eldönthető, hogy a kód romlott el vagy a teszt rossz._
