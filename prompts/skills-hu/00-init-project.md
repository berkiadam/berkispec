...
- **`.gitignore` — a `test-runs/` bejegyzés (KT7, csak VCS esetén):** ha a projektnek van verziókezelője, ellenőrizd a `.gitignore`-t:

  ```bash
  grep -qxF 'test-runs/' .gitignore 2>/dev/null && echo "MAR_BENNE" || echo "NINCS_BENNE"
  ```

  Ha `NINCS_BENNE`, ajánld fel a felvételét: <!-- INCLUDE:lang/00-init-project.md#KT7-gitignore-felajanlas --> Csak **jóváhagyás után** írj a `.gitignore`-ba, és **pontosan ezt az egy sort** vedd fel: `test-runs/`. Ha nemet mond, rögzítsd, és **ne kérdezd újra**. _(Ez a `/bs-run-tests` cikluson kívüli futtatásainak eredmény-fája: gépfüggő, újragenerálható, és a D8 szerint soha nem ciklus-bizonyíték — a verziókezelésben csak zaj.)_
...
2.c **A `## <sec:cv_test_execution>` szekció kitöltött (KT1)?** — van kategória-szótár a `**<field:f_test_categories>:**` mezőben, a projekt-szintű futtatási tábla minden kategóriájához valós parancs tartozik, és a `### Tesztfájl-helyek` tábla minden kategóriához glob-ot (vagy explicit `—`-t) ad. Enélkül a `/bs-run-tests` nem tud futtatni, a `08` teszt-leltár kapuja (`test-inventory-check.py`) pedig nem tud felderíteni.
