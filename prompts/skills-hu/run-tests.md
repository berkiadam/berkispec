...
> **Test manager (opcionális, TM4) — a cikluson kívüli futás is feltölthet.** Ha a `conventions.md` `## <sec:cv_test_reporting>` szekciójának `**<field:f_test_manager_phases>:**` mezője felsorolja az `ad-hoc` értéket, a futás **előtt** `--mode preflight`, **után** `--mode publish` fut (`test-manager.py --phase ad-hoc --round-dir <a futás-mappa>`). **Ez NEM kerüli meg a bizonyíték-tűzfalat (D8/KT6):** a feltöltés metaadata `cycle=none`, tehát a szolgáltatónál is megkülönböztethető marad a kényelmi futás a ciklus-bizonyítéktól. Trend- és flaky-adatnak épp ez a legértékesebb forrás.
