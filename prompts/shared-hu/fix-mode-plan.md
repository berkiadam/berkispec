...
### Két belépési alak
1. **Közvetlen javítás:** a `<status:must_fix>` megállapítás a plant érinti (a célfázis 03) — célzottan javítod.
...
### Bemenet
- A planre szűrt `<status:must_fix>` lista (kategória + leírás + `fájl:hely`), vagy reconciliation esetén a megváltozott upstream (spec) összefoglalója.
...
A `<status:must_fix>`-et, amihez **valódi döntés** kell, **ne találd ki** — vedd fel új `Knn`-ként a `plan-questions.md` végére, és **ne kérdezd közvetlenül a felhasználót** (fix-módban nincs interaktív csatornád). A kérdezést az orchestrátor (`05-analyze`) végzi. (A határvonal ugyanaz, mint a normál plan-írásban: megfigyelhető viselkedést vagy technológiai alapdöntést érintő kérdést nem döntesz el magad.)

### <field:f_status> (auto, `[analyze-loop]` marker)
A hurok a `plan.md` státuszát `[analyze-loop]` markerrel nyitotta vissza (pl. `<status:draft> [analyze-loop]`). Amíg a marker jelen van, **automatikusan** lépteted a státuszt, megerősítés-kérés nélkül:
- van nyitott `[ ]` kérdés a `plan-questions.md`-ben → `<status:open_questions> [analyze-loop]`;
- minden kérdés `[x]`, minden szekció rendben, minden schema artifact `<status:reviewed>`, a célzott javítás kész → `<status:ready_for_tasks> [analyze-loop]`.
...
Adj vissza tömör összefoglalót: (a) mely `<status:must_fix>`-eket / spec-változásokat vezettél át és hogyan, (b) milyen új `Knn` kérdéseket vettél fel a `plan-questions.md`-be (azonosítóval). A `plan.md`-t és a `plan-questions.md`-t te írod; az `analyze-report.md`-t **nem** — az az orchestrátoré.
