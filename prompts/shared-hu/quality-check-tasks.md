...
- **🔴 Teszt-hivatkozás minden teszt-taskon (TI2):** minden teszt-író `[RED]` és minden teszt-futtató `[CHECK]` sor végén ott a `— test [TC-01]` / `— test [TS-03]` jelölés, **létező** plan-azonosítóval? (Nem teszt-jellegű ellenőrzésen — typecheck, build, lint — nincs rá szükség.)
- **🔴 Minden futtatandó teszt külön checkbox (TX1):** egy teszt-futtató `[CHECK]` **pontosan egy** azonosítót futtat, a parancs pedig a teszt-szűrőt is tartalmazza (`-t "<név>"`, `-k <minta>`)? Nincs olyan sor, amely mögé több teszteset van összegyűjtve („futtasd a unit teszteket") — abból egy bukásnál nem derül ki, melyik teszt bukott.
- **🔴 `<sec:test_coverage>` tábla teljes (TT1):** a plan **minden** `TS-NN` forgatókönyve és a `<sec:machine_run_table>` **minden** kategóriája szerepel a táblában, megnevezve a **létrehozó** és a **futtató** taskot? Ahol mindkettő `—`, ott van egy mondatos indok (kézi lépés / `validate`-fázisú, a 07 futtatja)? A `<sec:plan_coverage>` tábla ezt **nem** helyettesíti: az a `[P-…]` szekciókat számolja el, a teszteseteket nem.
- **🔴 A `[CHECK]` parancsa a plan adatlapjából jön, és nem ütközik (T6):** ahol a plan `<field:f_test_run>` sora egy fájlra szűkített parancsot ad, a task azt viszi tovább (nem a teljes suite-ot), és **két `[CHECK]` nem irányít `>`-tal ugyanabba a log-/riportfájlba** — a második felülírná az elsőt, és a fázis végén egyetlen bizonyíték maradna. Gyűjtőfájlnál `>>`, egyébként kategóriánként külön fájl.
- **🔴 A belépő kapu (EG1) lefutott?** — A fázis elején tényleg lefuttattad az `analyze-gate-check.py --plan-only`-t a ciklusra, és `0`-t adott? A `plan.md` státusz-mezője **önbevallás**: ha a kapu bukott, itt nem lehet tasks lista, hanem vissza a `03`-ra.
