...
- **Mechanikus kapu (M):** lefutott az `analyze-gate-check.py` a ciklus mappájára, és `0`-t adott? (A `Must Fix` tételek gépiesen kimutatott hibák — státuszváltás előtt javítandók, nem a 05 hurkára hagyva.)
