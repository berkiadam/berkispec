...
| `dod-check.py` | 07 | Joins the DoD points with the round's JUnit results through the `· _evidence:_` fields (DI1/DI2). A test case has **three** states: a `<skipped>` case is not a `PASS` but `?` — a silently skipped test is not evidence (`SK1`). | yes |
...
| `validate-gate-check.py` | 07 | The collective gate: statuses, open tasks/DoD ticks, IP1 items, open Must Fix findings, round block ↔ folder match, round coverage (`RUN1`: every `validate`-phase category of the plan's machine table appears in the round's `results.json`; a missing `results.json` means the round was not driven from the table), skip evidence (`SK1`: a `skipped` case that the plan marks as `TC-NN` evidence), and the scope of the REST log (`RL1`: does a `rest-logs/remote/<test>/` log really contain a non-local address — a declared port-forward address is exempt; `RL2`: the label ↔ evidence join — every `[remote]` scenario has a log in the round). | yes |
...
| `ci-run-skill.sh` | 09a–09d | The adapter of the centralized path (L13-D12/L13-D13): `ci-run-skill.sh <skill> <cycle-path>` → `exit 0` green / `1` failure / `2` a human decision is needed. One script, four platform branches (Claude Code · Cursor · Copilot · Antigravity) + a `command` escape hatch, chosen by the `CI agent:` field of `conventions.md`. **The verdict comes from the deterministic gates, not from the agent** (measured: `cursor-agent` exits with `0` even after an authentication failure). **🔴 Measured on 2026-09-22: the Antigravity CLI (1.107.0) has no headless mode** — on a CI the `command` branch is the honest answer, and the selftest says so. `--selftest` is run by `00-init-project` so that an obsolete switch surfaces when the project starts, not on a live PR. | yes |
...
| `spec-plan.md` | `quick-flow` | **Only on the simplified route:** the merged spec + technical outline of the cycle, in place of the `spec.md` + `plan.md` pair (QF21), in a folder named `cycle-NN_quick-flow_<cycle-name>` (QF22). A cycle started before the rename keeps its `spec.md` and its folder name; the gates accept both. |
